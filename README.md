...
<br>
이 프로그램은 대학생들이 하루의 목표를 설정하고 성취도를 시각적으로 확인할 수 있는 **'자기관리 루틴 시스템'**입니다.
...
2. 할 일 목록 추가 및 개수 관리  
3. 실시간 달성률(%) 계산 및 출력  
4. 달성 점수에 따른 동기부여 등급 판별  
5. 주간/월간 누적 데이터 조회 및 분석  
6. 프로그램 종료
...
<br>
1. 내용 1: '투두 메이트' 컨셉 도출  
*프롬프트 요약: "파이썬으로 대학생들이 실생활에서 흥미를 느끼며 코딩할 수 있는 독창적인 프로젝트 주제 추천해 줘."  
*적용 내용: AI가 제안한 여러 아이디어 중 **'갓생 살기: 투두 메이트'**라는 컨셉을 채택함. 단순히 할 일을 나열하는 기능을 넘어, 나의 하루 성취도를 데이터로 확인하고 '갓생'을 인증한다는 목표를 세우니 기획과 구현 과정이 훨씬 몰입도 있게 느껴짐.
<br>
2. 내용 2: 로드맵 도출  
*프롬프트 요약: "1차 과제 필수 요건(변수 5개, 자료형 3개 이상)을 충족하기 위한 투두 메이트 데이터 구조 모델링 논의"  
*적용 내용: 단순히 '할 일' 개수만 받는 대신, 데이터 특성에 맞춰 **문자열(str - 이름, 날짜), 정수형(int - 총 개수, 완료 개수), 실수형(float - 목표 집중 시간)**으로 변수를 명확히 세분화하여 입력 데이터의 구조적 완성도를 높임.
<br>
**증빙 자료  
[1차 AI협업캡처.pdf 첨부 완료]
